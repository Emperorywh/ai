...
- `src/` 下 32 个 TypeScript 生产源码文件；
- `test/` 下 10 个测试与测试支撑文件；
...
- 状态：`FileStateStore`；
- 独占锁：`FileRunLock`；
- Agent：`ClaudeAgentSdkExecutor + ConsoleClaudeMessageObserver`，前者执行协议，后者实时投影终端动态；
- 门禁：`NodeGateRunner`；
- 提示词：`PromptBuilder`；
- 时钟：`SystemClock`；
...
| `validate` | `-m, --manifest` | 加载并完整校验 Manifest、DAG、声明路径和所有引用内容，输出项目根、任务数、稳定任务队列和内容哈希 |
...
5. 稳定排序全部 TASK；
6. 创建初始 RunState；
7. 原子保存状态并记录 `run_started`，终端同时展示任务数量和稳定任务队列；
8. 驱动任务直到运行终止或收到中断；
9. 无论成功失败都释放锁。
...
- 启用 `includePartialMessages`，把 assistant 文本 token 增量、工具调用、工具结果、工具耗时、系统状态和终态逐条交给消息观察器；
- 生产组合根使用 `ConsoleClaudeMessageObserver` 实时输出，输出带 TASK/attempt 前缀；thinking 正文不外显，只展示 token 进度；
- Claude Code 子进程 stderr 在保留尾部用于错误映射的同时，也会立即输出到终端；
- 观察器属于旁路能力，输出失败只报告诊断，不改变 Agent 结果和 checkpoint；
...
- tracked diff 显式使用 `--relative`，保证父仓库子项目与仓库根项目都统一返回项目根相对路径；
- 路径去重、统一为 `/` 并排序；
...
终端应用事件为人类可读单行，JSONL 保存完整事件。组合日志器并行写两个目标；某个日志器失败时向 stderr 报告，但不会让运行失败。Claude SDK 的高频动态使用独立消息观察器直接输出，不创建状态 checkpoint，也不写入应用事件 JSONL。
...
| `git-workspace.test.ts` | 暂存前后指纹稳定、父仓库子项目路径基准、候选变化拒绝、提交 trailer 精确恢复 |
...
| `claude-agent-sdk-executor.test.ts` | 预中止、init 回调、中止优先、无 session 失败、无 result、增量消息转发、Draft-07、登录来源、stderr |
| `console-claude-message-observer.test.ts` | assistant 增量文本、工具动态、推理进度、终态与实时 stderr 的终端投影 |
...
14. **任务目录不会被隐式扫描。** 运行队列只来自 Manifest 的 `tasks` 数组；仅创建 `tasks/TASK-002.md` 不会把它加入运行，必须同时在 Manifest 中显式声明依赖、scope 和 gates。`validate`、`run_started` 和 `run_resumed` 都会展示实际稳定队列。
...
| `infrastructure/claude/claude-message-observer.ts` | SDK 消息流与实时展示之间的基础设施内部观察边界 |
| `infrastructure/claude/console-claude-message-observer.ts` | assistant、工具、状态、终态和 stderr 的实时终端投影 |

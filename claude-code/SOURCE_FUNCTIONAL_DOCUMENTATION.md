...
- `claude-model-resolver.ts`：从同一用户设置源投影 attempt 使用的标准模型字段。
...
每个新 attempt 从 Claude 用户配置的标准 `model` 字段读取一次模型并先行持久化；恢复已有 attempt 不重新解析，也不维护环境变量或 SDK 默认模型 fallback。`system/init.model` 必须精确等于该 attempt 的请求模型，不一致返回不可重试 `model_mismatch`，关闭 Query，并将请求/实际模型保存到 attempt。

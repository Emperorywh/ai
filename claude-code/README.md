...
可写 Worker 阅读规格、任务和项目代码，完成实现与非浏览器验证
          │
          ├─ Worker 报告阻塞 → 冻结现状并由全新只读 Reviewer 独立审计
          │                    ├─ 仍有可执行工作 → 交回 Worker 修复
          │                    └─ 确认真实外部依赖 → 停止当前 Run
          │
          ▼
冻结本次代码候选及其内容指纹
...
`resume` 省略 Run ID 时不会向前搜索其他状态，只读取 `latest`。Reviewer 返回 `blocked` 时，Apex 会先确认工作区没有其他改动，从 quarantine 引用恢复候选并重新冻结恢复后的指纹，再用新的只读 Reviewer 会话继续审核；这也包括 Reviewer 已确认的 Worker 阻塞报告。对于旧版本遗留的“Worker 直接 blocked、没有 Reviewer、归档明确无变更”状态，Apex 会要求工作区完全干净，迁移为待审计阻塞报告并进入相同审核流程；存在未归档候选时不会猜测恢复。资源耗尽、失败和已完成 Run 仍保持终态。如需恢复其他 Run，请显式传入它的 Run ID。
...
Worker 返回 `blocked` 时不会直接进入终态，而会先冻结当前候选并进入独立阻塞审计：

```text
executing
  → candidate_pending
  → reviewing
  → retry_pending（阻塞不成立）/ blocked（Reviewer 确认真实外部依赖）
```
...
Reviewer 产生的 `blocked` 是可显式重开的审核终态：修复审核策略或补齐外部事实后执行 `resume`，系统会恢复隔离候选并继续审核。Worker 自身报告的阻塞必须先被 Reviewer 确认，因此不会再凭单次 Worker 会话终结 Run；资源预算耗尽不会被自动重开。
...
- Worker 报告 `blocked` 时，Reviewer 审计该声明而不是审核完成候选；阻塞不成立则进入 repair，阻塞成立才形成终态，`approved` 不能把未完成候选送入提交。

...
- 每个 worktree 基于任务启动时的主分支基线创建；合并流程顺序为：**先 rebase 到最新主分支**（rebase 会重写 commit hash），**rebase 完成后**由 Orchestrator 采集 post-rebase 的执行 commit 元信息（`hash / message / author / 时间`，即主分支历史中实际存在的 commit hash），再把这些元信息写入 `.result.md` 的 `execution_commits` 字段并以独立的 workflow audit commit 提交，**最后**以 fast-forward 合并方式回收到主分支，避免无意义的 merge commit。`execution_commits` 只记录 Task Executor 产生的实现 commit，不包含 Orchestrator 为回填审计字段而创建的 audit commit；audit commit 只作为记录载体进入主分支历史。审计元信息必须在 rebase 之后、fast-forward 之前回填，确保记录的 `hash` 与主分支历史一致；rebase 前的旧 hash 一律丢弃，不作为审计依据。
- 合并顺序由 application 层按 `depends_on` 拓扑序决定，先合并被依赖方，再合并依赖方；任何任务都不得先于其依赖任务回收到主分支。
- 合并冲突由 **Orchestrator 负责协调**，不交给 Task Executor 自行解决；冲突无法自动消解时，将后合并的任务置为 `blocked`，并把冲突清单写入 `docs/ISSUES.md`。
- `docs/PROGRESS.md`、`docs/DECISIONS.md`、`docs/ISSUES.md`、任务 frontmatter 状态等**全局工作流状态只在主分支由 Orchestrator 维护**，worktree 中只读引用；Task Executor 完成后只把需要更新的内容写入 `.result.md` 的结构化字段，由 Orchestrator 在合并回主分支时统一回写到全局文档，避免多 worktree 并发写同一文件。
- 并行任务的全局文档一致性：Orchestrator 在调度并行任务时记录各自的主分支基线 commit，Executor 读到的全局文档以该基线为准；合并回写时 Orchestrator 按 `depends_on` 拓扑序**串行**处理，每次回写前基于最新主分支重读全局文档再合并 `global_update_requests`。回写采用**机器可判定的 section 级合并**：每条 `progress` 更新必须声明 `mode`（`replace` 整段替换目标 section / `append` 拼接到目标 section 末尾）和目标 `section`；`append` 按拓扑序拼接，不同 `section` 互不影响直接合并；多条 `replace` 命中同一 `section` 时按拓扑序后写者覆盖先写者，先写者落选并由 Orchestrator 将冲突项写入 `docs/ISSUES.md`，视情况把后合并任务置为 `blocked`，后写者不得静默覆盖先写者。`decisions` / `issues` 更新按 Orchestrator 统一分配的 `id` 去重追加，不参与 section 合并。
...
- 先定义模块边界，再拆分任务。
- 优先长期架构正确性，而不是短期完成速度。
- 不保留 legacy 兼容代码、deprecated 旧入口、灰度开关或隐式 fallback；网络重试、错误提示、环境能力检测等运行时容错必须作为显式错误处理或能力声明写入 `docs/SPEC.md` / `docs/ARCHITECTURE.md`，不得作为兼容旧系统的隐藏逻辑存在。
- 不通过临时 patch 解决结构性问题。
- 每个任务只负责一个清晰目标。
...
- 对 `docs/SPEC.md` 与 `docs/ARCHITECTURE.md` 做生成后自动一致性校验（结构完整、字段合规、内部引用一致），再提交 Reviewer 独立审查；该自动校验不替代 Reviewer 的独立审查。
- 生成 `docs/PLAN.md`。
- 拆分 `docs/tasks/`。
- 维护任务依赖关系。
- 生成每个任务的上下文包。
...
Task Executor 不应依赖历史聊天记录，也不应执行当前任务以外的后续任务；除当前任务 `.result.md` 外，不得直接更新 `docs/PROGRESS.md`、`docs/DECISIONS.md`、`docs/ISSUES.md` 或任务 frontmatter 状态。
...
Reviewer 在任务拆分前还负责独立审查 `docs/SPEC.md` 和 `docs/ARCHITECTURE.md`，避免 Orchestrator 自审。只有 SPEC 与 ARCHITECTURE 通过 Reviewer 审查后，Orchestrator 才能进入生成 `docs/PLAN.md` 与 `docs/tasks/` 阶段。

任务执行结果的审查结论由 Reviewer 写入独立的 `docs/tasks/TASK-XXX-xxx.review.md`（模板见第 15 节），不写入 `.result.md`；`.result.md` 只记录 Task Executor 的执行事实，审查结论与执行事实分别由不同角色拥有，互不覆写。当任务声明 `no_review: true` 时，Reviewer 不介入，由 Orchestrator 生成 `review_result: skipped` 的 `.review.md`。
...
路径约定：`AGENTS.md` 位于项目根目录，承载 agent 通用执行约束；其余长期工作流文档统一位于 `docs/`，任务文档统一位于 `docs/tasks/`。所有 Context Pack、文档仓储、CLI 命令和任务 frontmatter 都必须使用这套路径约定，不得在根目录 `SPEC.md` / `ARCHITECTURE.md` 与 `docs/SPEC.md` / `docs/ARCHITECTURE.md` 之间混用。

自举例外：当一个新系统在实现本工作流自身，且当前只存在一份已经人工确认的合并需求文档时，可以把该文档作为 `source_spec`（例如本仓库的 `Readme.md`）。该例外必须在 PLAN 中显式记录，并在首个 bootstrap 任务中生成薄 `docs/ARCHITECTURE.md` 与 `docs/PROGRESS.md`，同时把 `source_spec` 作为权威规格来源写入架构文档。此例外只用于自举，不改变目标项目通过 `docs/SPEC.md` + `docs/ARCHITECTURE.md` 承载长期协议的常规规则。
...
为支持按 `layer` 裁剪验证范围（见第 16 节验证 allowlist），`TESTING.md` 中每条自动验证命令可选择性声明 `layers`（适用的 `layer` 枚举值列表，见第 9 节）；未声明 `layers` 的命令对所有任务生效。每条命令还可以声明 `requires_permissions`，用于显式列出该命令除 `run_commands` 以外需要的额外能力（例如 `install_dependencies`、`network_access`、`start_dev_server`）。该声明使用 YAML frontmatter 或 fenced YAML block 表达；系统不得依赖命令字符串启发式来授予权限，启发式检查最多用于产生告警。
...
- `AGENTS.md`、`docs/ARCHITECTURE.md`、`docs/PROGRESS.md`、当前任务文件为**必读核心**，任何任务都要带；当前任务文件是 Context Pack 的入口载体，本身不计入 `required_docs` 数组，但属于必读核心。
- `docs/SPEC.md`、`docs/PLAN.md`、`docs/DECISIONS.md`、`docs/ISSUES.md`、`docs/TESTING.md` 为**按需引用**，由 Orchestrator 根据任务 `layer` 和 `allowed_paths` 选择与本任务相关的章节或全文注入，无关章节不注入，避免上下文污染。自举项目若使用 `source_spec` 例外，则按需引用该 `source_spec` 的相关章节。
- 源码文件由 Orchestrator 圈定，依据为 `allowed_paths`、`docs/ARCHITECTURE.md` 模块边界，以及 `depends_on` 对应任务**已完成时**其 `.result.md` 的 `modified_files` / `created_files`。拆分阶段依赖尚未执行，先按依赖任务的 `allowed_paths` 预填 `source_files`；任务转入 `running` 前若依赖已完成，Orchestrator 用实际 `.result.md` 清单刷新该字段并回写 frontmatter，再启动 Executor；executor 不得自行扩展范围。
...
context_pack:
  ...
  source_files: []
workflow_outputs:
  result_file: docs/tasks/TASK-XXX-xxx.result.md
---
...
- docs/PROGRESS.md
- docs/tasks/TASK-XXX-xxx.md
- Orchestrator 在 `context_pack.optional_doc_excerpts` 中声明的 `docs/SPEC.md` / `docs/PLAN.md` / `docs/DECISIONS.md` / `docs/ISSUES.md` / `docs/TESTING.md` 相关章节
...
- docs/tasks/TASK-XXX-xxx.result.md
- 在 `.result.md` 中写入 `docs/PROGRESS.md` 更新建议
- 在 `.result.md` 中写入 `docs/DECISIONS.md` 更新建议，如有新增架构决策
- 在 `.result.md` 中写入 `docs/ISSUES.md` 更新建议，如有未解决问题

> 合并回主分支后，由 Orchestrator 统一回写 `docs/PROGRESS.md`、`docs/DECISIONS.md` 和 `docs/ISSUES.md`，这是 Orchestrator 的后续动作，不属于 Task Executor 的产出。
```
...
列出建议由 Orchestrator 回写到 `docs/PROGRESS.md`、`docs/DECISIONS.md`、`docs/ISSUES.md` 的内容；Task Executor 不直接修改这些全局文档。
```
...
global_update_requests:
  progress:
    - section: "当前完成到哪个任务"      # 对应 docs/PROGRESS.md 的章节
...
4. Reviewer 独立审核规格与架构；**未通过则回到第 2/3 步由 Orchestrator 修订后重提**，Reviewer 驳回意见写入 `docs/ISSUES.md`、修订记录写入 `docs/DECISIONS.md`，直至通过才进入第 5 步。
5. 生成 `docs/PLAN.md`。
6. 拆分 `docs/tasks/`，并为每个任务把 Context Pack 初始写入 frontmatter（`source_files` 按依赖 `allowed_paths` 预填，见第 8 节）。
...
10. 全部任务进入 `done` 后执行项目收尾：跑 `docs/TESTING.md` 全量验证、把 `docs/PROGRESS.md` 更新为终态快照（标注项目完成与当前可用能力全集）、归档或标记 `docs/ISSUES.md` 遗留项，由 Orchestrator 或人工决定是否打 tag / 发版。

自举项目若使用第 6 节定义的 `source_spec` 例外，第 2/3 步可以改为“确认 `source_spec` 并生成薄 `docs/ARCHITECTURE.md`”；第 4 步仍必须完成独立审查或人工确认，确认结果写入 PLAN 的前置说明，且不得遗留会阻塞实现的待确认问题。该例外不允许跳过后续任务级 Reviewer gate。
...
- 明确不会修改哪些模块。
- 必须遵守的架构边界。
- 从 `docs/PROGRESS.md` 继承到的上下文。
- 发现的风险或不确定点。
- 预计执行步骤。

如果发现当前任务与 `docs/SPEC.md`、`docs/ARCHITECTURE.md`、`docs/PLAN.md` 或 `docs/PROGRESS.md` 冲突，或发现需要越过 `forbidden_paths` 才能修复的架构问题，必须先指出冲突，并在 `.result.md` 中记录阻塞原因和全局文档更新建议；是否修改文档、扩权或重开任务由 Orchestrator 或用户确认，不应直接编码。
...
- 只完成当前任务，不提前实现后续任务。
- 不破坏 `docs/ARCHITECTURE.md` 中定义的模块边界。
...
审查结论由 Reviewer 写入独立的 `docs/tasks/TASK-XXX-xxx.review.md`（不写入 `.result.md`），其模板如下：
...
- 验证命令来源与优先级：项目级 `docs/TESTING.md` 与任务级 frontmatter `verification` 共同构成**验证 allowlist**，取**并集**。`docs/TESTING.md` 中每条自动验证命令可声明 `layers`（适用的 `layer` 枚举，见第 9 节；未声明表示对所有 layer 生效）；任务实际执行的项目级命令 = `layers` 未声明 ∪ `layers` 包含本任务 `layer`，再与任务级 `verification` 取并集，避免无关任务背上全量验证开销。同一命令在两处声明时以任务级为准。
- 验证 allowlist 内的命令执行时自动获得**仅限该具体命令行**的执行授权，无需在 `permissions` 中重复声明 `run_commands`；该命令对 `allowed_paths` 内文件的读写副作用同样自动允许。
- 验证命令若涉及 `allowed_paths` 之外的能力，仍需在 `permissions` 中显式声明对应能力：安装依赖 `install_dependencies`、联网 `network_access`、启动长期服务 `start_dev_server`、打开浏览器 `open_browser`、删除文件 `delete_files`、修改配置 `modify_config`。任务编写者应在 `docs/TESTING.md` 的命令声明或任务 frontmatter 的 `verification` 附近显式记录每条命令的 `requires_permissions`；若执行时检测到命令声明缺失能力，命令因权限不足失败，Task Executor 应将 `execution_status` 标为 `blocked` 并在 `.result.md` 中说明，而不是自行扩权。
- 命令能力不得通过魔法字符串授权。系统可以用启发式检查提示“该命令看起来需要额外权限”，但最终授权只以 `permissions` 与命令级 `requires_permissions` 的交集校验为准。
...
任务失败时必须将以下信息写入 `docs/tasks/TASK-XXX-xxx.result.md`（与正常完成的结果文件同路径，frontmatter 的 `execution_status` 标注为 `failed` 或 `blocked`），并在 `global_update_requests.issues` 中登记建议由 Orchestrator 回写到 `docs/ISSUES.md` 的阻塞项：
...
为避免人工确认点密集成为自动化瓶颈，系统应支持：批量确认（一次处理多个 `blocked` / `needs-human` 任务）、带超时的默认放行（超时未响应时按任务声明的默认 `next_action` 处理并记审计）、以及 `docs/ISSUES.md` 阻塞项按 `severity` 与停留时长的升级提醒。具体策略由 `application` 层调度配置决定，本文档不强制。

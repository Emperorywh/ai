...
要求：verification 由你实跑验证命令后自报 command / result / notes；source / 退出码 / 耗时 / 输出摘要由 Orchestrator 系统验证补全，模型不产；executionStatus 与 nextAction 组合须合法（completed+review / blocked+needs-human 或 retry / failed+needs-human 或 retry）；多余文本不得出现在 JSON 块内。`
...
/**
 * 归一化模型自报 verification：补 source='model' + 系统验证四元组空值（TASK-039）。
 *
 * 模型产出的 JSON 经 SdkResultJsonSchema（复用 ResultVerificationSchema，新字段 optional）校验，
 * 系统验证四元组（exit_code / duration_ms / output_summary）可能缺省——模型无法知道真实退出码 / 耗时。
 * 此处统一补全为 source='model' + 空值，使落盘的模型自报记录字段完整、来源明确：模型自报记录一律标
 * source='model'（模型不得自报 source='system' 伪造系统验证），待 VerifyTaskUseCase 经系统 Runner
 * 覆盖为 source='system' 真实记录（FR-011.5）。
 */
function normalizeModelVerification(records: readonly ResultVerification[]): ResultVerification[] {
...
function mapToSdkRunReport(data: SdkResultJson, report: SdkSessionReport): SdkRunReport {
  ...
  return {
    ...
    verification: normalizeModelVerification(data.verification),

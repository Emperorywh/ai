 ...
 *
 * 本文件实现 TaskReviewerPort（application/execution/ports.ts，TASK-036 收敛）的真实调用类
 * ClaudeSdkReviewer：经 sdk-client（TASK-030）的 runSdkSession 起一次**独立审查会话**（与执行
 * 会话分离，§5 / Readme §5.2-5.3 职责分离，不共享对话历史），以 input.result（.result.md
 * frontmatter）+ worktree 内实际改动（模型用 Read / Bash(git diff) 自读）为审查对象，prompt
 * 要求模型对照 Readme §15 审查清单产出 JSON → ReviewOutcome。
 *
 ...
 * 成功映射 ReviewOutcome。
 *
 * 分层定位（ARCHITECTURE §3 / 任务 §8）：infrastructure adapter——依赖 sdk-client（同层）+
 * claude-sdk-invocation-impl（同层，复用 classifyFault 容错分类纯函数）+ core Schema（type +
 * 运行时校验）+ **application/execution/ports.ts 的审查契约类型**（ReviewInput / ReviewOutcome /
 * TaskReviewerPort）。TASK-036 把审查契约从 CLI 收敛到 application 后，本实现直接 import 共享
 * 类型作为方法签名（依赖倒置：infrastructure → application 仅限 ports），不再本地维护结构对齐
 * 的重复类型（SdkReviewInput / SdkReviewOutcome 已删除）。provider 配置（env/model）经构造函数
 * 注入（同 TASK-032 模式，env/model 来自 TASK-031 组装产物）。
 *
 ...
 * ReviewOutcome 接口（application/execution/ports.ts），无需 camelCase→snake_case 转换，直接 safeParse。
 *
 ...
 */
function buildReviewPrompt(input: ReviewInput, feedback: string): string {
  const parts = [
...
/**
 * 构造降级 ReviewOutcome（§4.3 parse 耗尽 / §8 容错 / §9 中断的统一降级形态）。
 *
 ...
 */
function degradedOutcome(input: ReviewInput, reason: string): ReviewOutcome {
  return {
...
/** 成功路径：模型 JSON（snake_case）→ ReviewOutcome（纯映射）。 */
function mapToReviewOutcome(data: ReviewJson): ReviewOutcome {
  return {
 ...
 *
 * 经 `implements TaskReviewerPort` 在编译期证明结构满足 application 审查 Port（任务 §11）。
 *
 * review(ReviewInput) 链路（任务 §9）：
 ...
 */
export class ClaudeSdkReviewer implements TaskReviewerPort {

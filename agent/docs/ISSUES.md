...
提议自 `TASK-002-core-enums.result.md`。TASK-004 已基于该推断集实现 `DecisionSchema` / `IssueSchema` 并通过验收，确认后只需回写 Readme，无需改代码。
...
提议自 `TASK-003-core-task-schema.result.md`。裁定方向决定 DEC-002 是否需要回写。
...
提议自 `TASK-004-core-decision-issue-schema.result.md`。裁定方向决定 DEC-003 是否需要回写或开新任务。

...
## DEC-034 ClaudeSdkReviewer 设计——独立审查会话 + snake_case 模型 JSON（对齐 ReviewOutcome）+ review-frontmatter 标记 + 复用 classifyFault + 结构类型兼容 Reviewer 契约（不 import cli）
...
## DEC-037 task:review composition root 装配策略（assembleReviewer reviewerKind 三态 + auto key 缺失回退 LocalReviewer 不报错）+ CI 真实 API 契约策略（describe.skipIf 有 key 跑/无 key skip 标注 + 断言契约不断言文本）
...
status: proposed
scope: src/cli/commands/task-review.ts（assembleReviewer / reviewTaskWithAssembly / registerTaskReviewCommand）+ test/integration/claude-sdk-real-api.test.ts + .github/workflows/ci.yml
created_from_task: TASK-035
decision: |-
  TASK-035 把审查侧 SDK 装配收敛为独立 assembleReviewer(input) → { reviewer, observability }（SPEC §6/§13.2 composition root），与 reviewTask 编排解耦（reviewTask 仍只接受 options.reviewer，TASK-027 既有注入点不变；CLI action 经 reviewTaskWithAssembly 串联）。reviewerKind 三态：local（不读 token 直回退 LocalReviewer）/ sdk（显式 SDK，token 缺失抛错不回退，对称 task:run --executor sdk）/ 省略=auto（读 profile，token 就位走 SDK）。**与 task:run（assembleExecutor，DEC-035）的关键差异**：task:run auto + token 缺失 = 报错（执行必须用 SDK，DryRun 是显式选项，SPEC §14.3）；task:review auto + key 缺失/配置缺失 = catch 回退 LocalReviewer + 显著告警（§12 兜底 + ISS-016 不静默放行）不报错——因 review 有 LocalReviewer 合法兜底，且使既有 task-review.test.ts 的 runCli 默认路径测试（TASK-461/462，不在本任务 allowed_paths 不可改）无需 --reviewer 标注即不破。--model 作具体模型名直写 reviewer.model（SPEC §6）。CI 真实 API 契约策略（SPEC §11/§14-7/8）：vitest describe.skipIf(!hasKey) 条件运行——有 ANTHROPIC_API_KEY/ZHIPU_API_KEY 时跑最小审查任务（ClaudeSdkReviewer 真实调用，断言 review_result ∈ 合法枚举 + system init model 反映档位映射，不断言文本）；无 key 时经 describe.skipIf(hasKey) 反向 describe 跑「skip 显式标注」测试（断言 hasKey===false + console.warn 标注），不静默通过。CI workflow 分两 job：baseline（typecheck/lint/test 无真实 API）+ real-api-contract（受 secret 控制，无 key 自动 skip）。复用判断：task-run.ts 虽在 forbidden_paths，但 forbidden 约束的是「修改」非「依赖」——task-review.ts 自 TASK-027 起即 import task-run.ts（先例），本任务扩展该 import 追加 createObservability/CostSummary/Observability（TASK-034 已导出），符合 AGENTS §3 DRY。
rationale: |-
  对称复用 TASK-034 装配模式（DEC-035 assembleExecutor + DEC-036 createObservability）最大化代码复用 + 降低审查侧接线的实现风险。auto 回退 LocalReviewer 不报错（而非对称 task:run 报错）是 task:review 特有设计：(1) §12 明文「SDK 未就位用本地审查器兜底，避免阻塞」，LocalReviewer 是合法兜底而非降级；(2) 既有 task-review.test.ts 13 项测试（TASK-027 产出）含两个 runCli 默认路径测试（TASK-461 approved→done、TASK-462 非 reviewing→非零），TASK-035 的 allowed_paths 不含该测试文件（不可改），auto 报错会破这些测试（临时仓库无 .caw/config.json），auto 回退 LocalReviewer 使其不破；(3) 使 caw task:review <id> 在无 provider 配置环境仍可跑通，不阻断既有工作流。--reviewer sdk 显式 + key 缺失仍报错（用户显式要 SDK 但没配好是错误），保持与 task:run --executor sdk 对称。CI 真实 API 契约用 describe.skipIf（vitest 原生条件运行）而非手写 if-else：有 key 自动启用、无 key 自动 skip，且经反向 describe 跑「skip 显式标注」测试避免静默通过（SPEC §14-7 验收）。真实 API 契约直接测 ClaudeSdkReviewer（TASK-035 接线注入对象）而非完整 task:review CLI 链路：SdkReviewOutcome review_result ∈ 枚举即等价 .review.md 过 ReviewFrontmatterSchema（review_result 是 Schema 枚举字段），无需 reviewing git/worktree 夹具（过重）。considered alternatives：装配塞进 reviewTask（破坏纯编排 + 既有测试全改）；auto 报错对称 task:run（破既有测试 + 阻断无配置环境）；就地重实现 createObservability（违反 DRY，ISS-015 既抱怨）；真实 API 用完整 CLI 链路（夹具过重）。
consequences: |-
  task-review.ts 新增 ~230 行（装配 + CLI 选项），reviewTask 主体零改动。CLI 默认行为变化：caw task:review <id>（无 --reviewer）从 TASK-027 静默 LocalReviewer 变为 auto 读 profile（token 就位走真实 ClaudeSdkReviewer，缺失回退 LocalReviewer + 显著告警）。TaskReviewOutcome 增可选 cost?: CostSummary（LocalReviewer undefined，向后兼容）。项目首次有 CI（.github/workflows/ci.yml）：baseline + real-api-contract 两 job，push/PR 触发。真实 API 契约断言未经本地验证（无 key，ISS-024）——首次 CI 配置 secret 跑通时需确认 system init model 字段提取路径（R-API）+ GLM 档位前缀（R-PROVIDER）+ classifyFault 真实错误措辞（ISS-022 同延）。后续若抽 cli 共享助手模块（ISS-015），assembleReviewer 与 assembleExecutor 可统一收口。关联 DEC-035（task:run 装配策略，本任务对称复用）/ DEC-036（§7 createObservability，import 复用）/ DEC-034（ClaudeSdkReviewer 设计）/ DEC-033（ClaudeSdkInvocation 同模式参照）。
```

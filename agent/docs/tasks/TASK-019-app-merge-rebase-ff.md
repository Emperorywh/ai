...
来自 PLAN P6。合并回收主分支的核心步骤（§3.2）：按拓扑序 → 先 rebase 到最新主分支 → rebase 后回填 post-rebase 的 `execution_commits`（hash/message/author/time）→ fast-forward 合并，避免 merge commit。

## 2. 当前目标

实现 `rebaseAndFastForward(adapter, task, { mainRef })`：
- 按拓扑序逐任务：rebase worktree 分支到最新 main。
- rebase 成功后回填 `.result.md` 的 `execution_commits`（post-rebase hash）。
...
worktree 分支 → rebase → 回填 execution_commits → ff 回收 main → 产出（成功 | 冲突清单）。
...
- 临时 git 仓库集成测试：rebase + ff 后 main 历史无 merge commit；execution_commits hash 与 main 一致。

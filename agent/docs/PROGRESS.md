...
- Core 四份 type 资产齐备（枚举 + 任务 Schema + 决策 / 问题 Schema + 执行结果 Schema），可被 frontmatter 解析（TASK-010）、全局文档读写（TASK-012）、SQLite 索引（TASK-014）、状态机（TASK-007）、状态映射（TASK-008）复用：
...
- 工具链就绪：`npm run typecheck` / `npm test` / `npm run lint` / `npm run build` 均可执行且全绿（140 项单测）。
...
- TASK-006：Core 审查结论 Schema（`.review.md`）（layer: `type`，depends_on: TASK-002 已完成）。复用 `enums.ts` 的 `ReviewResultSchema` 等定义 `.review.md` frontmatter Schema，沿用既有「Zod schema 单一来源 + `z.infer` 派生 + 复用 enums」模式。

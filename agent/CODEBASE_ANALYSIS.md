...
- 权限模式为 `dontAsk`；
- 通过 `tools: []` 显式关闭全部内置工具；
- 不额外设置 SDK Turn 上限，结构化输出校验与有限重试由 SDK 负责；
- 通过结构化输出返回结果；
- 不指定固定模型，使用 SDK 当前配置决定的模型；
- 鉴权和 Claude 环境配置也由 SDK/运行环境提供，项目代码不自行管理密钥。
...
- 任务只含最小元数据、需求和验收标准；
- 更新状态不会丢失任务正文。
- 访谈与规划显式关闭内置工具，并且不会被单轮上限截断结构化输出流程。
- 终端多行回答保留换行、缩进和空白行，多轮回答不会串流，输入结束行为明确。

这里的“13 个测试用例”中部分用例同时验证多个行为，所以上述覆盖点多于 13 项。

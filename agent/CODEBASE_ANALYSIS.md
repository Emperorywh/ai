...
4. CLI 使用 Readline 读取用户回答：手动 Enter 直接提交，bracketed paste 块中的换行作为答案内容保留；
5. 将问题和回答显式加入访谈记录；
...
- 终端粘贴边界使用标准 bracketed paste 控制序列识别，不依赖输入速度或时间阈值；
...
- 保持 Readline `question()` 的 Enter 直接提交体验；
- 将输入流保护的粘贴换行还原为完整访谈回答；
- 隔离终端提示符和应用层访谈输入端口。

`src/cli/bracketed-paste-input.ts`：

- 启用并解析终端标准 bracketed paste 边界；
- 通过流式状态机处理跨输入块的边界、UTF-8 字符和 CRLF；
- 只保护粘贴块内部的换行，普通 Enter 继续交给 Readline 提交；
- 在命令结束时关闭终端粘贴模式，避免污染后续 PowerShell 会话。
...
- `interview` 无论成功失败都会关闭 Readline、恢复终端模式并销毁粘贴输入流；
...
- 普通回答按一次 Enter 提交，粘贴边界可以跨输入块，粘贴换行、CRLF 和原始标记字符均能无损还原。

这里的“18 个测试用例”中部分用例同时验证多个行为，所以上述覆盖点多于 18 项。

...
需求访谈和任务规划不属于 CAW 运行时能力。`caw init` 生成两份跨 AI 工具提示词，用户可以选择任意 AI 工具生成规格和任务；CAW 从标准文件读取事实，每次启动新的 Claude Code 会话执行一个任务。
...
每次任务执行调用新的 `query()`，不设置 `resume` 或 `continue`，并设置 `persistSession: false`。
